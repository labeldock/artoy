...
export function isAbsoluteNaN (it) {
...
export function asArray (data, defaultArray) {
  if (isArray(data)) {
      return data;
  }
  if (isNone(data)) {
  ...
  if (typeof data === 'object' && typeof data.toArray === 'export function') {
      return data.toArray();
  }
  return [data];
};
...
export function browse (obj, path) {
...
export function findGet (data, findFn, getPath) {
  return get(asArray(data).find(findFn), getPath);
};
// 대상 array에 직접 index에 해당하는 값을 삭제합니다.
export function removeIndex (data, index) {
...
export function removeValue (data, value) {
  const asData = asArray(data);
  const valueIndexes = [];
...
export function removedIndex (data, index) {
  return removeIndex(toArray(data), index);
};

export function generateUUID (pattern = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'){
...
export function encode64 (plainText=""){
...
export function deferralReference (value){
...
export function parseMessagePayload (msg){
  try {
